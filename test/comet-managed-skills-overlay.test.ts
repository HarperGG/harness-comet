...
const harnessBlock = /\n?<!-- HARNESS-COMET:BEGIN [^\n]+ -->[\s\S]*?<!-- HARNESS-COMET:END [^\n]+ -->\n?/g;

function stripHarnessBlocks(content: string): string {
  return content.replace(harnessBlock, "");
}
    ...
    for (const phase of phases) {
      it(`${language}/comet-${phase} preserves upstream text byte-for-byte`, async () => {
        const [managed, upstream] = await Promise.all([
          readFile(
            path.join(managedRoot, language, `comet-${phase}`, "SKILL.md"),
            "utf8"
          ),
          readFile(
            path.join(snapshotRoot, language, `comet-${phase}`, "SKILL.md"),
            "utf8"
          )
        ]);
