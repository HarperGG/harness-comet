---
name: comet-open
description: "Comet 阶段 1：开启。用 /comet-open 调用。完整保留官方中文 Open 工作流，并集成 Playwright 影响分析与编写决策。"
---
...
官方中文上游基线：https://github.com/rpamis/comet/tree/master/assets/skills-zh/comet-open
...
- 一个让用户 **自行输入名称** 的明确选项
- 提示：若用户输入中文或其他非 kebab-case 文本，会被转换为合规的 kebab-case 英文名，并在使用前回显给用户确认

不得在用户确认最终 change 名称前运行 `openspec new change` 或创建 `.comet.yaml`。若选定或转换后的名称与已有 change 冲突，必须报告冲突并请用户另选名称。
...
如果用户已确认澄清摘要，直接使用该摘要填充产物内容。如果不存在澄清摘要，回退到技能默认行为并询问用户。
...
2. 获取产物指令：`openspec instructions <artifact-id> --change "<name>" --json`
3. 读取 `dependencies` 中列出的每个已完成依赖产物
4. 以 `template` 作为产物结构并遵循 `instruction`
5. 将 `context` 和 `rules` 作为约束条件应用，**不得复制到 artifact 内容中**
6. 写入 `resolvedOutputPath`
7. 验证输出文件存在且非空
8. 每创建一个 artifact 后，重新运行 status 确认状态，再继续下一个 artifact

**失败处理：** 如果 `openspec instructions` 失败、返回无效 JSON、报告未满足的 `dependencies`，或未提供可用的 `resolvedOutputPath`，必须立即停止 artifact 创建并报告 OpenSpec 错误。不得回退为硬编码文档结构。

**命名与范围守卫：** change name 必须使用 Step 1c 中用户确认的 kebab-case 英文名，不得自动生成、推断或使用非 kebab-case 名称。变更范围必须与用户描述一致，不得自行扩大或缩小。
...
**文件存在性验证：** 逐个确认三个文件路径存在且非空。任一文件缺失或为空时，不得进入下一步或执行阶段守卫，必须回到创建步骤补充。
...
### 6. 用户审视确认与 Playwright 编写决策（阻塞点）

三个文档创建完成、内容完整性检查和影响分析通过后，必须按 `comet/reference/decision-point.md` 的协议暂停并等待用户确认。不得在用户确认前执行阶段守卫或自动流转。
...
- Playwright 行为变化、受影响测试、覆盖缺口和建议

非 Playwright 项目保留官方单选项：
...
用户选择「需要调整」或请求调整时，按其说明修改对应产物，按需重新执行完整性检查和影响分析，然后重新请求确认。

### 7. Harness-Comet Open Gate

Playwright 模式下，在官方 Comet 阶段守卫前执行：
...
- Playwright 模式下，Harness-Comet Open gate 已通过
- 官方 Comet 阶段守卫使用 `--apply` 通过
...
hotfix/tweak preset 由对应 preset skill 控制后续流转。批量拆分模式下，不得自动推进单个拆分项；等待用户选择要开始的 change。
